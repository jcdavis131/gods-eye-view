// The default metro brief: what changed, with the arithmetic.
//
// The place page is the acquisition channel and carries the query intent; the
// brief is the retention channel, reached by an in-content link from the page
// and by a feed reader. No brief URL appears in any sitemap, and a brief that
// found nothing is served, linked and followed but not offered for indexing —
// "nothing crossed a line" is true and useful to a subscriber and thin to a
// crawler.
//
// Metro briefs are thinner than county briefs by construction, because there
// is no QCEW metro path: their findings come from Zillow's metro home value
// and rent rows, the county rollup estimate, and nearby crossings and ports.
// The brief says so in a caveat rather than reading as a quiet month.
...
import { parseMetroParam, scopePath, type PlaceScope } from "@/lib/places/scope";
...
async function load(cbsa: string): Promise<{ scope: PlaceScope; facts: Awaited<ReturnType<typeof placeFacts>>; brief: Brief }> {
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    ...
    // A brief with no findings is followed, not indexed: it is a real page for
    // a subscriber and a thin one for a crawler.
...
export default async function MetroBriefPage({ params }: Params) {
  const { cbsa } = await params;
  const { scope, facts, brief } = await load(cbsa);
    ...
    <article>
      ...
      <header className="mt-2">
        ...
        <p className="mt-2 max-w-[48rem] text-[15px] leading-relaxed">
          ...
          , with the occupation mix and every section&rsquo;s source.
        </p>
      </header>
