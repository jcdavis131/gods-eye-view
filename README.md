...
# NWS alerts (polygon or listed counties), current GDACS events (quakes USGS also has carry alsoInUsgs), EONET volcanoes
...
export const myLayer: LayerDefinition = {
  ...
  dependsOn: ["earthquakes"],     // optional: other layers whose on/off state fetch() reads as ctx.layersOn
  ...
  async fetch(ctx: FetchContext): Promise<FetchResult> {
    ...
    // ctx.options = settings prefs, ctx.layersOn = the dependsOn layers' on/off state
